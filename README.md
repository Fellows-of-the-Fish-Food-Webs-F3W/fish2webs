...
4. Compute structural and trophic metrics at both the local food-web and node (i.e. fish trophic species) levels.
...
`size2webs()` uses the individual fish data generated by `fish2size()` to construct a global size-structured metaweb and operation-specific local food webs. Structural and trophic metrics are computed for each local food web and for each fish trophic species within each sampling operation.
...
| `trophic_species_level_metrics` | Structural and trophic metrics computed for each fish trophic species within each sampling operation |
